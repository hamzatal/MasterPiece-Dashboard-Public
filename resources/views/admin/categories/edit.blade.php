        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-2xl sm:rounded-2xl">
                <div class="p-8 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <form method="POST" action="{{ route('categories.update', $category->id) }}" enctype="multipart/form-data" class="grid grid-cols-1 gap-6">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="w-32 h-32 relative group">
                                    @if ($category->image)
                                        <img
                                            ...
                                            alt="{{ $category->name }}"
                                            class="w-full h-full object-cover rounded-lg shadow-md group-hover:opacity-70 transition duration-300"
                                        >
                                        <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300">
                                            <span class="text-white text-xs bg-black bg-opacity-50 px-2 py-1 rounded">
                                                {{ __('Current Image') }}
                                            </span>
                                        </div>
                                    @else
